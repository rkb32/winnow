...
**A full audit of Imagenette's official split** (`imagenette_exp/full_audit.py`, `imagenette_exp/full_audit/verified_pairs.csv`). Every one of the 9,469 official training photos was compared against every one of the 3,925 official validation photos — 37.2 million pairs — in 34 minutes on 14 CPU cores, using the same detectors as the live pipeline (multiprocessing for per-image hashing/embedding, a vectorized popcount Hamming distance, and a matrix-multiply for embedding similarity). It flagged 108 pairs.

Every flagged pair was then independently judged by three AI reviewers (Claude agents reading the actual images, not humans) with different adversarial framings — neutral, a skeptic hunting for differences, a matcher hunting for shared details — and the 6 pairs they disagreed on were escalated to a fourth agent reading the full-resolution originals. Every verdict and its visual evidence is recorded in the CSV, so any individual pair can be re-checked by a person.
